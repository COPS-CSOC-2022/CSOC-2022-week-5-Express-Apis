...
middlewareObj.addUserToRequest = function (req, res, next) {
  Token.findOne(
    ...
    (err, token) => {
      ...
      User.findById(token.user, "_id name email username", (_err, user) => {
        if (user == null) {
...
middlewareObj.hasCorrectTitle = function (req, res, next) {
  ...
  if (req.body.title.trim() === "") {
    return res.status(400).json({
      error:
        "The title of the todo cannot be blank or just made up of whitespaces.",
    });
  }
  next();
};
...
middlewareObj.hasCollaborator = function (req, res, next) {
  ...
  if (typeof username != "string" || !usernameRegex.test(req.body.collaborator))
    return res.status(400).json({
