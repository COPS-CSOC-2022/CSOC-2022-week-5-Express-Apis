    ...
    (err,foundTodos)=>{
      if(!err){
        if(foundTodos){
          ...
          console.log(users);
          User.find({_id: {$in: users}},(err,foundUsers)=>{
            if(!err){
              if(foundUsers)
                res.status(200).json({
...
const deleteToDo = async (req, res) => {
  ...
  ToDo.findByIdAndDelete(ToDoId, (err)=>{
    if(err){
      res.status(500).send(err);
    }else{
      ...
      if(!err){
        if(userFound){
          ...
          if(userFound.id==req.user.id){
            return res.status(401).send("Todo creater cannot be Todo collaborator");
          }
              ...
              if(!error){
                if(foundTodo){
                  ...
                  if(foundTodo.collab && foundTodo.collab.includes(userFound.id)){
                    return res.status(401).send("Given user is already a collaborator");
                  }
                if(foundTodo.collab){
  ...
  User.findOne({username:collaborator},(err,userFound)=>{
      if(!err){
        if(userFound){
          console.log(userFound);
          if(userFound.id==req.user.id){
            return res.status(401).send("Todo creater was not a Todo collaborator");
          }
            ToDo.findById(ToDoId,(error,foundTodo)=>{
              if(!error){
                if(foundTodo){
                  ...
                  if(foundTodo.collab && foundTodo.collab.includes(userFound.id)){
                    ...
                    return res.status(400).send("No such collaborator");
                  }
