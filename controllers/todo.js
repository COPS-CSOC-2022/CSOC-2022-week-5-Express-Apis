...
const getAllToDo = (req, res) => {
  ToDo.find({
    ...
    .then((todos) => {
      ...
      const collabTodos = todos.filter(
...
const getParticularToDo = (req, res) => {
...
const editToDo = (req, res) => {
  ToDo.findById(req.params.id, (err, todo) => {
    ...
    if (
      !todo.createdBy.equals(req.user.id) &&
...
const addCollaborator = (req, res) => {
  ToDo.findById(req.params.id, (err, todo) => {
    ...
    User.findOne({ username: req.body.collaborator }, (err, collab) => {
      ...
      if (collab._id.equals(req.user._id))
        return res.status(422).json({
          error: "Creator of the todo cannot be added as collaborator",
        });

      if (todo.collaborators && todo.collaborators.includes(collab._id))
